...
local function isEligiblePlayer(player)
    return
        player and player:isAlive() and not player:isDead() and not player:isAsleep() and player:getCurrentSquare() ~=
            nil
end
...
local function processDelayedConverts()
    if DelayedConverts.count == 0 then
        return
    end
        ...
        if entry.framesLeft <= 0 then
            ...
            if zombie and not zombie:isDead() then
                ...
                if onlineID >= 0 then
                    ...
                    log("Sent NemesisConvert for onlineID=" .. tostring(onlineID) .. " outfit=" ..
                            tostring(entry.outfitName))
                end
            end
...
local function convertToNemesis(zombie, player)
    ...
    if pid then
        ...
        if decisions then
            ...
            if not globalData.zombies then
                globalData.zombies = {}
            end
