...
local function registerNemesisClientModule()
    ...
    RegionManager.ZombieModules.register({
        ...
        sounds = {
            ...
            theme = {
                name = "ApocNemesis_Theme",
                range = themeRange,
                loop = true
            },
            onDetect = {
                name = "ApocNemesis_STARS",
                range = starsRange
            },
            periodic = {{
...
local function onNemesisConvertCommand(module, command, args)
    ...
    for i = 0, zombieList:size() - 1 do
        ...
        if zombie and not zombie:isDead() and zombie:getOnlineID() == zombieID then
            ...
            print("[ApocNemesisBoss] Client: converted zombie " .. tostring(zombieID) .. " to Nemesis (outfit=" ..
                      tostring(outfitName) .. ", getOutfitName=" .. tostring(zombie:getOutfitName()) .. ", tough=" ..
                      tostring(modData.Apocalipse_TSY_ToughnessType) .. ", maxHits=" ..
                      tostring(modData.Apocalipse_TSY_ToughnessMaxHits) .. ")")
            break
        end
    end
end
...
local function isNemesisOutfit(outfit)
    ...
    if cachedOutfitSet[outfit] then
        return true
    end
...
local function scrubLeakedNemesisToughness(zombie)
    ...
    print("[ApocNemesisBoss] Failsafe: scrubbed leaked Nemesis toughness from " .. "zombie onlineID=" ..
              tostring(zombie:getOnlineID()) .. " outfit=" .. tostring(outfitName))
    return true
end
